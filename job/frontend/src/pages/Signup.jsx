...
const Signup = () => {
    ...
    const inputs = [
        {
            ...
            errorMessage: "campany name shouldn't contain capital letter",
            label: "Name",
            pattern: "[a-z]*",
            required: true,
        },
...

        {
            ...
            placeholder: "",
            pattern: "123",
            errorMessage: "password should be 123",
            label: "Password",
            required: true
        },
    ...
    const handleSubmit = (e) => {
        ...
        const url = "http://localhost:8080/api/v1/campany/add";
        try {
            fetch(url, {
                method: 'POST',
    ...
    return (
        ...
        <div className=' bg-gray-100 max-w-md flex flex-col mx-auto mt-10 p-6 '>
            ...
            <form action='' onSubmit={handleSubmit}>
                {inputs.map((input) => (
                    <SignupFormInput key={input.id} {...input} value={values[input.name]} onChange={onChange} />
                ))}

                <p class="mt-4 text-sm text-gray-600 dark:text-gray-400">
                    Already have an account?
                    <a class="text-blue-600 decoration-2 hover:underline font-medium ml-2" href="http://localhost:5173/signin">
                        Sign In here
                    </a>
                </p>
                <div className='flex justify-center'>
                    <button type="submit" class="py-2 px-4 mt-8 text-center justify-center rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none transition-all text-sm">Sign up</button>
                </div>
            </form>
...
//                                                 </div>
//                                             </div>
//                                             <p class="hidden text-xs text-red-600 mt-2" id="name-error">Please include a valid campany name*</p>
//                                         </div>
