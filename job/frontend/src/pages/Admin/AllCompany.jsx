import { useState } from "react";
import { useEffect } from "react";

const AllCompany = () => {

    const [companies, setComapny] = useState([]);


    useEffect(() => {
        const url = "http://localhost:8080/api/v1/campany/retrive";
        fetch(url).then((result) => result.json()).then((campany) => {
            setComapny(campany);
        }
        ).catch(err => {
          console.log(err);
        })
      },[]);


    const deleteCompany = async (companyId) => {
        
    };
    return (
        <div>
            <h1 className='text-4xl font-bold m-20 h-'>Company</h1>

            {companies != null && companies.length === 0 ? (
                <p className='text-xl font-medium m-20'>No Company</p>
            ) : (
                <ul>
                    {companies.map((item) => (
                        <div key={item._id} className="flex flex-row flex-wrap mx-10">
                            <div className='m-3 md:m-8'>
                                <img className="rounded-t-lg w-[192px] h-[280px]" src={item.img ? item.img : "data:image/png;base64,"} alt={item.title} />
                            </div>
                            <div className='m-3 space-y-4 md:m-8'>
                                <h2 className='text-4xl font-medium'>{item.username}</h2>
                                <p className="text-xl">Email: {item.email}</p>
                                <p className="text-xl">Name: ${item.Name}</p>
                                <button type="button" className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600" onClick={() => deleteCompany(item._id)}>Remove</button>
                            </div>
                        </div>
                    ))}
                </ul>
            )}
        </div>
    );
};

export default AllCompany;